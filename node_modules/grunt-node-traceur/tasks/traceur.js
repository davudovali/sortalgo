...
// 'import' statements.
try {
  var modPath = path.resolve('node_modules/grunt-node-traceur/node_modules/traceur/src/node/inline-module');
...
function buildToFile(grunt, options, f) {
  ...
  // strip the code wrapper since node runs everything in its own context
  //code = code.slice(0, code.length - includeData.split('\n').length - 3);
  for (var ln = 0; ln < code.length; ln++) {
    var line = code[ln];
    if (line.match(/^var \$__src_.*? = /) ||
      // these are safe due to whitespace traceur adds to the users code.
      line == "  return Object.preventExtensions(Object.create(null, {}));" ||
      line == '}).call(this);'
      ) {
      code[ln] = '';
    }
  ...
  if (code.indexOf('$traceurRuntime') != -1) {
    ...
    if (!options.browser) {
      code = 'require("' + pkg.name + '/runtime");' + code;
      console.log("Traceur runtime required. Please ensure grunt-node-traceur is saved as a dependency.");
    } else {
      console.log("Traceur runtime required: node_modules/grunt-node-traceur/runtime.js");
      console.log("Please include it for browser usage. Node usage will auto require it.");
    }
  }
  code += '\n';

  // Fix mappings, we skipped first 2 lines
  //sourceMapGenerator._mappings.forEach(function(mapping) {
